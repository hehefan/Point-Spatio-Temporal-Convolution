...
class MSRAction3D(Dataset):
    def __init__(self, root, frames_per_clip=16, frame_inverval=1, num_points=2048, train=True):
        ...
        for video_name in os.listdir(root):
            if train and (int(video_name.split('_')[1].split('s')[1]) <= 5):
                ...
                for t in range(0, nframes-frame_inverval*(frames_per_clip-1)):
        ...
        self.frame_inverval = frame_inverval
